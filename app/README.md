...
2. Start the FastAPI server:
   ...
   python server.py
   ```
   *The server will load the PyTorch models into memory (which may take a few moments) and start listening on `http://0.0.0.0:8000`.*
...
3. Start the development server:
   ...
   pnpm run dev --host
   ```
   *The `--host` flag exposes the app on your local network, allowing you to test it from your mobile phone!*
